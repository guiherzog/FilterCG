...
public class ImageConvolution
{
  ...
  public static ArrayData convolute(ArrayData inputData, ArrayData filter, int kernelDivisor)
  {
  ...
  public static void main(String[] args) throws IOException
  {
    ...
    int weight = 5;
    int filterNormalizer = filterWidth*filterHeight*weight - 1;
    
    System.out.println("Filter size: " + filterWidth + "x" + filterHeight +
                       ", divisor=" + filterNormalizer + ", weight="+weight);
    
    ArrayData filter = new ArrayData(filterWidth, filterHeight);
    for (int i = 0; i < filterHeight; i++)
    {
      System.out.print("[");
      for (int j = 0; j < filterWidth; j++)
      {
        filter.set(j, i, weight);
        System.out.print(" " + filter.get(j, i) + " ");
      }
      System.out.println("]");
    }
    
    ArrayData[] dataArrays = getArrayDatasFromImage(imgInput);
    int filterIterations = 10;
